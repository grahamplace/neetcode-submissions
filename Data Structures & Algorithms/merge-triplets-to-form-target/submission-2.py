...
class Solution:
    ...
    def compress(self, triplets: List[List[int]]) -> List[List[int]]:
        assert len(triplets) >= 2, "Unexpected merge call"
        i, j = 0, 1
        new_item = [
        ...
        # return [t for idx, t in enumerate(triplets) if idx not in [i, j]] + [new_item]
        return [new_item] + triplets[2:]
