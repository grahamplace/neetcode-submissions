...
class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None:
        if userId not in self.tweets_by_users:
            self.tweets_by_users[userId] = [(self.tweet_counter, tweetId)]
        else:
            tweets = self.tweets_by_users[userId]
            if len(tweets) == self.MAX_SIZE:
                heappop(self.tweets_by_users[userId])

            heappush(tweets, (self.tweet_counter, tweetId))
