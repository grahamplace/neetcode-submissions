...
class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        while left < right:
            ...
            move_left_new_sum = numbers[left + 1] + numbers[right]
            move_right_new_sum = numbers[left] + numbers[right - 1]

            # make the move that keeps the max sum (under target), so that we step thru all possible valid sums in decreasing order until we hit target

            # if moving left exceeds target, we should never do that move
            if move_left_new_sum > target:
                right -= 1 

            elif move_left_new_sum >= move_right_new_sum:
                left += 1
            else:
                right -= 1
