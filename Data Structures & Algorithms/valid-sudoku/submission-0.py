...
class Solution:
    ...
    def isValidSubGrid(self, board: List[List[str]], r: int, c: int) -> bool:
    ...
    def solvePoint(self, board: List[List[str]], r: int, c: int) -> bool:
        return (
            self.isValidRow(board, r)
            and self.isValidCol(board, c)
            and self.isValidSubGrid(board, r, c)
        )

    def isValidSudoku(self, board: List[List[str]]) -> bool:
        for r in range(len(board)):
            for c in range(len(board[r])):
                if not self.solvePoint(board, r, c):
                    return False
